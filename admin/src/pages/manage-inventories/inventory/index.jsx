...
import CargosList from "./cargo-list";
...
const Inventory = (props) => {
  ...
  useEffect(() => {
    axiosClient(
      `/admin/get_inventory?_id=${_id}&location=${location}&page=${page}`
    )
      .then((result) => {
        if (result.data) {
  ...
  return (
    <InventoryContext.Provider
      value={{ setInventory, setCount, page, location }}
    >
      ...
      {inventory.length > 0 && (
        ...
        >
          ...
          <Paginate
            ...
            onPageChange={(currentPage) => searchParams.set(currentPage)}
          />
        </div>
      )}
    </InventoryContext.Provider>
